defmodule Entry do
    ...
    def validate_entry(e, hash \\ nil) do
        ...
        if eh.height < 0 or eh.height > 0x7FFF_FFFF_FFFF_FFFF, do: throw(%{error: :height_out_of_range})
        if eh.slot < 0 or eh.slot > 0x7FFF_FFFF_FFFF_FFFF, do: throw(%{error: :slot_out_of_range})
        if eh.prev_slot < -1 or eh.prev_slot > 0x7FFF_FFFF_FFFF_FFFF, do: throw(%{error: :prev_slot_out_of_range})
        ...
        if eh.root_tx != root_tx(tx_hashes), do: throw(%{error: :root_tx_invalid})
    ...
    def build_next(seed, cur_entry, txus) do
        ...
        header = %{
            ...
            root_chain: MMR.root_chain(DB.MMR.chain_id(), mmr)
        }
    ...
    defp merkle_root(kvs) do
      RDB.hbsmt_root(kvs)
    end

    def root_tx(hashes) do
      merkle_root(root_tx_build(hashes))
    end
    ...
    def root_validator(validator_pks, last_change_height) do
      merkle_root(root_validator_build(validator_pks, last_change_height))
    end
    ...
    def proof_tx_included(entry_hash, tx_hash) do
      ...
      RDB.hbsmt_root_prove(kvs, nil, tx_hash)
    ...
    def proof_validators(entry_hash) do
      ...
      proof = RDB.hbsmt_root_prove(kvs, nil, "hash")
