...
pub fn has_permission(env: &mut crate::consensus::consensus_apply::ApplyEnv, symbol: &[u8], signer: &[u8]) -> bool {
    consensus::bic::permission::has(env, &bcat(&[b"coin:", symbol, b":permission"]), signer)
}
...
pub fn call_update_permission(env: &mut crate::consensus::consensus_apply::ApplyEnv, args: Vec<Vec<u8>>) {
    let update = consensus::bic::permission::decode_update(args, b"symbol", "invalid_symbol");
    validate_symbol(&update.identifier);
    if !exists(env, &update.identifier) {
        panic_any("symbol_doesnt_exist")
    }
    let permission_key = bcat(&[b"coin:", &update.identifier, b":permission"]);
...
pub fn call_create_and_mint(env: &mut crate::consensus::consensus_apply::ApplyEnv, args: Vec<Vec<u8>>) {
    ...
    if consensus::bic::nft::collection_exists(env, &symbol) {
        panic_any("symbol_conflicts_with_collection")
    }
