...
pub fn total_supply(env: &mut crate::consensus::consensus_apply::ApplyEnv, symbol: &[u8]) -> i128 {
    match kv_get(env, &bcat(&[b"coin:", symbol, b":totalSupply"])) {
        Some(amount) => std::str::from_utf8(&amount)
            .unwrap()
            .parse::<i128>()
            .unwrap_or_else(|_| panic_any("invalid_total_supply")),
        None => 0,
    }
}
...
pub fn call_transfer(env: &mut crate::consensus::consensus_apply::ApplyEnv, args: Vec<Vec<u8>>) {
    ...
    kv_increment(
        env,
        &bcat(&[b"account:", receiver, b":balance:", symbol]),
        amount,
    );
...
pub fn call_create_and_mint(
    ...
    let symbol: Vec<u8> = symbol_original
        .iter()
        .copied()
    ...
    let decimals = std::str::from_utf8(&decimals)
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
        .unwrap_or_else(|| panic_any("invalid_decimals"));
