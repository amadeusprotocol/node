...
impl Exsss {
    ...
    pub fn shuffle<T: Clone>(&mut self, slice: &mut [T]) {
        ...
        let mut keyed: Vec<(f64, T)> = slice
            .iter()
            .map(|val| (self.uniform_float(), val.clone()))
            .collect();
...
mod tests {
    ...
    fn test_random_sequence() {
        ...
        assert_eq!(
            randoms,
            vec![
                294, 431, 615, 198, 771, 458, 832, 264, 842, 111, 320, 936, 44, 92, 979, 44, 402,
                648, 714, 722
            ]
        );
    }
    ...
    fn test_shuffle_large() {
        ...
        let expected_first_50 = vec![
            74, 86, 38, 82, 89, 10, 84, 26, 98, 85, 34, 91, 87, 51, 93, 45, 41, 30, 17, 96, 28, 6,
            27, 78, 23, 25, 92, 18, 32, 39, 48, 22, 49, 1, 61, 9, 20, 95, 72, 79, 70, 33, 50, 42,
            77, 73, 81, 24, 60, 88,
        ];
