...
pub fn call_lock(env: &mut crate::consensus::consensus_apply::ApplyEnv, args: Vec<Vec<u8>>) {
    if !crate::consensus::bic::coin::exists(env, b"PRIME") {
        ...
        let v0 = &[
            149, 216, 55, 255, 29, 8, 239, 251, 139, 112, 30, 29, 199, 57, 90, 67, 198, 220, 101, 18, 228, 100, 100, 241, 43, 213, 221, 230, 253, 58, 231, 1,
...
pub fn call_unlock(env: &mut crate::consensus::consensus_apply::ApplyEnv, args: Vec<Vec<u8>>) {
    ...
    if env.caller_env.entry_epoch < unlock_epoch {
        ...
        let prime_points = unlock_amount.checked_mul(multiplier).unwrap_or_else(|| panic_any("prime_points_overflow"));
...
pub fn call_daily_checkin(env: &mut crate::consensus::consensus_apply::ApplyEnv, args: Vec<Vec<u8>>) {
    ...
    while let Some((next_key_suffix, val)) = kv_get_next(env, &prefix, &cursor) {
        ...
        let unlock_amount = vault_parts[3].as_slice();
        let unlock_amount = std::str::from_utf8(&unlock_amount).ok().and_then(|s| s.parse::<u64>().ok()).unwrap_or_else(|| panic_any("invalid_unlock_amount"));
        total_unlock_amount = total_unlock_amount.checked_add(unlock_amount).unwrap_or_else(|| panic_any("total_unlock_amount_overflow"));
    ...
    if delta == 0 || delta == 1 {
        kv_put(
            env,
            &bcat(&[b"bic:lockup_prime:next_checkin_epoch:", &env.caller_env.account_caller]),
            env.caller_env.entry_epoch.saturating_add(2).to_string().as_bytes(),
        );
        ...
        if streak >= 30 {
            ...
            let streak_bonus = daily_bonus.checked_mul(30).unwrap_or_else(|| panic_any("streak_bonus_overflow"));
