...
impl EncodeToTerm for Attestation {
    fn to_term(&self) -> Result<Term, &'static str> {
        Ok(Term::PropList(vec![
            (
                Term::Binary(b"entry_hash".to_vec()),
                Term::Binary(self.entry_hash.to_vec()),
            ),
            (
                Term::Binary(b"mutations_hash".to_vec()),
                Term::Binary(self.mutations_hash.to_vec()),
            ),
            (
                Term::Binary(b"signer".to_vec()),
                Term::Binary(self.signer.to_vec()),
            ),
            (
                Term::Binary(b"signature".to_vec()),
                Term::Binary(self.signature.to_vec()),
            ),
        ]))
    }
}
