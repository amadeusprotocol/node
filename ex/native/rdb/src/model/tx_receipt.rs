...
impl EncodeToTerm for TXReceipt {
    fn to_term(&self) -> Result<Term, &'static str> {
        Ok(Term::PropList(vec![
            (
                Term::Binary(b"txid".to_vec()),
                Term::Binary(self.txid.to_vec()),
            ),
            (Term::Binary(b"success".to_vec()), Term::Bool(self.success)),
            (
                Term::Binary(b"result".to_vec()),
                Term::Binary(self.result.to_vec()),
            ),
            (
                Term::Binary(b"exec_used".to_vec()),
                Term::Binary(self.exec_used.to_vec()),
            ),
            (
